...
struct RGB **read_bmp (const char *filename) {
    sprintf(filename_in, "%s_2.%s", filename, "bmp");
    FILE *fp;
...
void ColorTemperature(struct RGB **data,int index) {
    struct RGB **dataWarm = (struct RGB**)malloc(height * sizeof(struct RGB*));
    struct RGB **dataCool = (struct RGB**)malloc(height * sizeof(struct RGB*));

    for (int i = 0; i < height; i++) {
        dataWarm[i] = (struct RGB*)malloc(width * sizeof(struct RGB));
        dataCool[i] = (struct RGB*)malloc(width * sizeof(struct RGB));
    }
    
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            float rw = data[i][j].R*1.2; // 調整紅色通道
            float bw = data[i][j].B*0.8; // 調整藍色通道
            float rc = data[i][j].R*0.8; // 調整紅色通道
            float bc = data[i][j].B*1.2; // 調整藍色通道       
            dataWarm[i][j].R = (unsigned char)fmax(0, fmin(255, rw)); // 更新 dataWarm 的紅色值
            dataWarm[i][j].G = data[i][j].G;                         // 綠色通道保持不變
            dataWarm[i][j].B = (unsigned char)fmax(0, fmin(255, bw)); // 更新 dataWarm 的藍色值

            dataCool[i][j].R = (unsigned char)fmax(0, fmin(255, rc)); // 更新 dataWarm 的紅色值
            dataCool[i][j].G = data[i][j].G;                          // 綠色通道保持不變
            dataCool[i][j].B = (unsigned char)fmax(0, fmin(255, bc)); // 更新 dataWarm 的藍色值
        }
    }
    write_bmp(dataWarm,index+1,3);
    write_bmp(dataCool,index+1,4);
}
